...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, onAuthStateChanged, signOut, updateProfile, updatePassword, updateEmail, sendEmailVerification } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";
...
function renderLanding(){
  ...
  signup.append(
    ...
    el("div",{cls:"form-field"},[el("label",{},["Birthday"]), el("input",{type:"date",name:"birthday",required:true})]),
  ...
  signup.onsubmit = async e=>{
    ...
    try{
      ...
      alert("Signup successful!"); renderLanding();
    }catch(err){ alert(err.message); }
  }
}
...
async function renderDashboard(){
  ...
  const btnMode = el("button",{cls:"neon-btn"},"Light/Dark");
  top.append(brand,btnProfile,btnLogout,btnMode);
  frame.append(top);
  ...
  // Live Date & Time
  const dateTimeDiv = el("div",{cls:"card"});
  dash.prepend(dateTimeDiv);
  function updateDateTime(){ dateTimeDiv.textContent = new Date().toLocaleString(); }
  setInterval(updateDateTime,1000); updateDateTime();

  btnLogout.onclick = async()=>{ await signOut(auth); renderLanding(); }
  btnProfile.onclick = ()=>renderProfile();
  btnMode.onclick = ()=>document.body.classList.toggle("light-mode");
...
async function renderProfile(){
  ...
  const firstName = el("input",{cls:"form-field",value:u.firstName||"",placeholder:"First Name"});
  const lastName = el("input",{cls:"form-field",value:u.lastName||"",placeholder:"Last Name"});
  const emailInput = el("input",{cls:"form-field",value:u.email||"",placeholder:"Email"});
...
async function renderAdminDashboard(){
  ...
  for(const u of usersSnap.docs){
    ...
    card.querySelectorAll("button")[0].onclick = async()=>{ if(confirm("Delete this user?")){ await deleteDoc(doc(db,"users",u.id)); renderAdminDashboard(); } }
